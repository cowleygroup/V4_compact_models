...
class LinearMappingClass:
	...
	def __init__(self, lambda_spatialpool=0.01, lambda_mixing=0.0001, num_epochs_total=100, learning_rate=0.00001, alpha_ridge=60000., svd_factorize_flag=True):
	...
	def perform_cross_validation_with_early_stopping_for_ensemble(self, embeds, raw_responses, metric='r2_ER', verbose=True):
		...
		if True:
			...
			for ifold in range(num_folds):
				...
				for imember in range(num_members):
					...
					Ytrain = (Ytrain - np.mean(Ytrain,axis=1)[:,np.newaxis]) / np.std(Ytrain, axis=1)[:,np.newaxis]
					...
					for iepoch in range(self.num_epochs_total):
						...
						if verbose:
							print('member {:d}, fold {:d}, epoch {:d}'.format(imember, ifold, iepoch))

			responses_hat_val  = responses_hat_val / num_members
			responses_hat_test  = responses_hat_test / num_members

			# choose best epoch with validation data
			R2s_val_over_epochs = np.zeros((self.num_epochs_total,))
			for iepoch in range(self.num_epochs_total):
				R2s_val_over_epochs[iepoch] = np.median(self.compute_R2_ER(responses[:,:-4,:], responses_hat_val[iepoch][:,:-4]))
	...
	def perform_one_fold_with_early_stopping_for_ensemble(self, embeds, raw_responses, metric='r2_ER', verbose=True):
		# perform 8 fold nested cross-val (where 1/8 is val, 1/8 is test for each fold)
		#	-- basically, choose some number of epochs and keep the same across models
		#		(you can vary lambdas instead)
		# computes 4-fold cross-validation on features to predict responses
		...
		responses_train = np.nanmean(raw_responses, axis=2)  # (num_neurons, num_images)
		...
		if True:
			...
			responses_hat_val_ridgereg = np.zeros((num_neurons,inds_val.size))
			responses_hat_test_ridgereg = np.zeros((num_neurons,inds_test.size))

			responses_hat_val = np.zeros((self.num_epochs_total,num_neurons,inds_val.size))
			responses_hat_test = np.zeros((self.num_epochs_total,num_neurons,inds_test.size))

			Ytrain = responses_train[:,inds_train]
			Yval = raw_responses[:,inds_val,:]
			Ytest = raw_responses[:,inds_test,:]
			num_train_images = Ytrain.shape[1]
			...
			for imember in range(num_members):
				...
				ridger.fit(np.reshape(Xtrain, (num_images,-1)), Ytrain.T)
				num_val_images = Xval.shape[0]; num_test_images = Xtest.shape[0];
				responses_hat_val_ridgereg += 1/num_members * ridger.predict(np.reshape(Xval, (num_val_images,-1))).T
				responses_hat_test_ridgereg += 1/num_members * ridger.predict(np.reshape(Xtest, (num_test_images,-1))).T
				...
				for iepoch in range(self.num_epochs_total):
					...
					responses_hat_val[iepoch] += 1/num_members * self.get_model_predicted_responses(model, Xval)
					responses_hat_test[iepoch] += 1/num_members * self.get_model_predicted_responses(model, Xtest)

					if verbose:
						print('member {:d}, fold {:d}, epoch {:d}'.format(imember, ifold, iepoch))

			# plot ridge reg prediction
			if True:
				R2s_val = self.compute_R2_ER(Yval[:,:-4,:], responses_hat_val_ridgereg[:,:-4])
				R2s_test = self.compute_R2_ER(Ytest[:,:-4,:], responses_hat_test_ridgereg[:,:-4])
				print('ridge reg R2_val = {:f}, R2_test = {:f}'.format(np.median(R2s_val), np.median(R2s_test)))

			# choose best epoch with validation data
			R2s_val_over_epochs = np.zeros((self.num_epochs_total,))
			for iepoch in range(self.num_epochs_total):
				R2s_val_over_epochs[iepoch] = np.median(self.compute_R2_ER(Yval[:,:-4,:], responses_hat_val[iepoch][:,:-4]))

				if verbose == True:
					R2s_test = self.compute_R2_ER(Ytest[:,:-4,:], responses_hat_test[iepoch][:,:-4])
					print('epoch {:d}, R2_val = {:f}, R2_test = {:f}'.format(iepoch, R2s_val_over_epochs[iepoch], np.median(R2s_test)))
			iepoch_max = np.nanargmax(R2s_val_over_epochs)

			# compute test performance
			R2s_over_neurons = self.compute_R2_ER(Ytest[:,:-4,:], responses_hat_test[iepoch_max][:,:-4])
	...
	def initialize_factorized_linear_mapping(self, num_pixels, num_filters, num_neurons, model_weights=None):
		...
		if True:
			...
			# ### TESTING !!!
			# weights = model.get_weights()
			# weights[0] = 0.01 * weights[0] / np.std(weights[0])
			# weights[6] = 0.01 * weights[6] / np.std(weights[6])
			# model.set_weights(weights)
	...
	def initialize_with_svd_Betas(self, model, Xtrain, Ytrain):
		...
		if self.alpha_ridge is None:
			self.alpha_ridge = 0.01 * np.median(np.var(Xtrain, axis=0) * (num_images-1)**2)
			if np.isnan(self.alpha_ridge):
				self.alpha_ridge = 0.01
	...
	def get_model_predicted_responses(self, model, features):
		...
		for ibatch in range(0,num_images,batch_size):
			responses_hat.append(model(features[ibatch:ibatch+batch_size], training=False))
